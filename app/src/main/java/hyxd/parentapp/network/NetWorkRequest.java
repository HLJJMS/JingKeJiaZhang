package hyxd.parentapp.network;
...
//            jsonObject.put("data", data);
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                logLength("2222222222222222" + url, resultStr + "");
                //判断服务器返回
...
//                        resultStr = AESCoder.decrypt(parseObject.getString("data"), Constants.KEY);
    ...
    //异步请求
    public static void twoPostTaskRequest(final Context context, final Handler handler, final String url, String data, final RequestCallback requestCallback) {
        Log.d(TAGUrl, url);
        logLength(TAGData, data);
        if (!CommonWay.netWorkCheck(context)) {
            handler.post(new Runnable() {
        ...
        MediaType jsonType = MediaType.parse("application/json;charset=UTF-8");
        RequestBody requestBody = RequestBody.create(jsonType, jsonObject.toJSONString());
        Log.d("ccccccccccccc", requestBody.toString() + "            " + jsonObject.toString());
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    ...
                    String resultStr = "";
                    resultStr = response.body().string();
                    Log.d("dddddddddddddddddddd", resultStr);
                    //判断服务器的返回情况
                    if (ResultCheck.check(resultStr)) {
                        JSONObject jsonObject1 = JSONObject.parseObject(resultStr);
                            final String code = jsonObject1.getString("code");
                        ...
                        handler.post(new Runnable() {
                            ...
                            public void run() {
                                requestCallback.onRequestFailed("网络连接失败，请重试");
                                Log.d("3333333333333", "ffffffffffffffffff");
                            }
                        });
                    }
                } catch (Exception e) {
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            requestCallback.onRequestFailed("网络连接失败，请重试");
                            Log.d("334444444444444", "ffffttttttttttttff");
                        }
                    });
                }
    ...
    public static String twoGetRequest(final Context context, String url) {
        ...
        try {
            ...
            if (response.isSuccessful()) {
                resultStr = response.body().string();
                logLength("0000000000000000000" + url, resultStr + "");
            }else {
                resultStr = "请求失败";
            }
        } catch (Exception e) {
